#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""App174 のフロントページカスタマイズを front_portal.js/css に復元する"""
import base64
import json
import os
import sys
...
JS_FILE = project_root / 'kintone_app' / 'customizations' / 'front_portal.js'
CSS_FILE = project_root / 'kintone_app' / 'customizations' / 'front_portal.css'
...
def main():
    if not JS_FILE.exists() or not CSS_FILE.exists():
        print(f'❌ file missing: {JS_FILE} / {CSS_FILE}')
        sys.exit(1)

    js_key = upload_file(JS_FILE, 'text/javascript')
    css_key = upload_file(CSS_FILE, 'text/css')

    payload = {
        'app': str(APP_ID),
        'desktop': {
            'js': [
                {'type': 'FILE', 'file': {'fileKey': js_key}},
            ],
            'css': [
                {'type': 'FILE', 'file': {'fileKey': css_key}},
            ],
        },
        'mobile': {'js': [], 'css': []},
    }
