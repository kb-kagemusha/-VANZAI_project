"""App171（請求書）にPDF添付用FILEフィールドを追加する。

既にFILEフィールドが存在する場合は何もしない。
存在しない場合は `invoice_pdf` フィールドを追加して deploy する。
"""
...
def main() -> None:
    ...
    existing_file_codes = [
        code for code, prop in properties.items() if prop and prop.get("type") == "FILE"
    ]

    if existing_file_codes:
        print(f"✅ App{app_id} には既にFILEフィールドがあります: {', '.join(existing_file_codes)}")
        return

    new_code = "invoice_pdf"
    payload = {
        "app": app_id,
        "properties": {
            new_code: {
    ...
    print(f"✅ App{app_id} にFILEフィールド `{new_code}` を追加し、デプロイを開始しました。")
