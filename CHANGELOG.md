...
### Added
- **OCR・保存データ**: タブに未確定件数バッジを表示（「アップロード済み画像」タブの未整理バッジと同様）

## [0.11.34] - 2026-07-09

### Fixed
- **OCR・精算レシート**: 端末番号が2行折り返しで一部のみ読み取られた場合の復元を強化（`-af4C` を端末識別番号と誤認しない、`f22d625c6a7` の先頭 `f` 欠落補正、端末識別番号 + af4c 断片 + 末尾 UUID からの再構成）

## [0.11.33] - 2026-07-09

### Fixed
- **OCR・精算レシート**: 精算日時の年桁 5/6 混同（例: `2026/06/29` → `2025`）を補正。マルチパス OCR の複数候補から正しい年を採用し、単一候補でも本日に近い年へ補正

## [0.11.32] - 2026-07-09

### Improved
- **OCR・保存データ**: 保存データ一覧に「未確定のみ」ボタンを追加。確定済み以外の行だけをワンクリックで絞り込み可能に

## [0.11.31] - 2026-07-09

### Improved
- **OCR・PaygateSS**: ファイル名変更の導線を改善。「後で」選択時の案内通知、保存データ一覧のバナー、画像ファイル列の「整理／名前変更」、詳細画面の「推奨名に整理／ファイル名を変更」を追加

## [0.11.30] - 2026-07-09

### Fixed
- **OCR・PaygateSS**: ファイル名整理ポップアップが半透明で背面と重なり読めない問題を修正（不透明パネル・背景暗転を強化）

## [0.11.29] - 2026-07-09

### Fixed
- **OCR・PaygateSS**: 「要確認」フラグ（金額補正・日時 fuzzy 等）だけで「問題なしで確定」が押せない問題を修正。目視確認後は確定可能にし、金額推定・必須項目欠損・検証エラーのみブロック

## [0.11.28] - 2026-07-09

### Changed
- **OCR・PaygateSS**: 行の確定と画像ファイル名の変更を分離。確定時の自動リネームを廃止し、同一画像の全取引が確定したときのみリネーム確認ポップアップを表示
- **OCR・PaygateSS**: レビューモーダルに同一画像の確定進捗（例: 3/5 件）を表示。画像一覧に「名前未整理」バッジと「名前を整理」操作を追加

## [0.11.27] - 2026-07-08

### Fixed
- **OCR・アップロード**: 精算レシート選択時にプレビューが画面全体に拡大表示される問題を修正（64px サムネイル表示に統一）

## [0.11.26] - 2026-07-08

### Fixed
- **OCR・PaygateSS 決済方法**: QRコードを現金と誤認する重大バグを修正。他行の「現金」混入・広域ブロックの優先・信頼度100%表示（実際は別行の現金 OCR 行を平均していた）を解消

## [0.11.25] - 2026-07-08

### Added
- **OCR・アップロード済み画像**: サムネイル枠内にアップロード日時を表示（選択前のドロップゾーン含む）

### Fixed
- **OCR・精算レシート**: PAYGATE POS 行が UUID 向け OCR 補正で `PAY0ATE P0S` に壊れる問題を修正。分割行・9/800 誤読から現金売上/POS 売上を復元。長尺レシート向け売上帯 OCR を追加

## [0.11.24] - 2026-07-08

### Fixed
- **OCR・PaygateSS**: 決済方法「QRコード」の読取を強化。右寄せレイアウト・OCR行順の乱れ・0R/OR 等の誤読・決消方法ラベル typo に対応
...
### Fixed
- **OCR・外部アップロード画面**: トークン付きURL再アクセス時にラベル・有効期限を正しく取得・表示。5枚制限の注意文を赤文字に復元し、有効期限をページ最下部へ表示

## [0.11.22] - 2026-07-07

### Changed
- **OCR・外部アップロード画面**: タイトルを「画像アップロード」に変更。ラベル（青・太字）とメモ（黒・太字）をタイトル下に表示し、5枚制限の注意をページ最下部へ移動

## [0.11.21] - 2026-07-07

### Changed
- **OCR**: Paygate/精算レシート切替タブもデータタブと同様の一体枠で囲むよう統一

## [0.11.20] - 2026-07-07

### Fixed
- **OCR・外部アップロードリンク**: 有効期限指定のセレクトと期限日入力の高さを統一し、期限日選択時も「リンクを発行」ボタンが右隣に並ぶようレイアウト調整

## [0.11.19] - 2026-07-07

### Changed
- **OCR・データタブ**: タブとコンテンツを一体の枠で囲み、履歴見出し上の不要な境界線を解消

## [0.11.18] - 2026-07-07

### Changed
- **ダッシュボード**: 「月次一括生成」ボタンを「対象月」の右隣に配置
- **登録申請**: 申請詳細パネルの左余白・上下パディングを調整し文字の見切れを解消
- **実績一覧**: 対象月・ステータスフィルタの最大幅を画面の半分に制限
- **サイドバー**: 折りたたみ時の選択中アイコンを枠・光彩で強調表示

## [0.11.17] - 2026-07-07

### Added
- **OCR・外部アップロードリンク**: 有効期限を「日数」に加え「期限日（カレンダー）」でも指定可能に（期限日は当日23:59 JSTまで有効）

## [0.11.16] - 2026-07-07

### Changed
- **OCR**: Paygate/精算レシート切替タブとデータタブの見た目を改善（濃いグレー枠・非選択時も背景色付きでタブと識別しやすく）

## [0.11.15] - 2026-07-07

### Changed
- **OCR・外部アップロードリンク**: 「管理画面からの直接アップロードに枚数上限はありません。」の文言全体を赤文字に統一

## [0.11.14] - 2026-07-07

### Changed
- **OCR**: タブを「アップロード済み画像」「保存データ」「CSVとの照合」「自己申告データとの比較」の4つに拡張（精算レシートは先頭2タブ）
- **OCR・外部アップロードリンク**: パネルの縦幅を圧縮（フォーム2列・余白縮小・一覧スクロール）

## [0.11.13] - 2026-07-07

### Changed
- **OCR・Paygate**: アップロード済み画像の下に「保存データ」「CSVとの照合」「自己申告データとの比較」タブを追加し、表示を切り替え可能に

## [0.11.12] - 2026-07-07

### Changed
- **OCR・外部アップロードリンク**: 説明文を「管理画面からの直接アップロード」の直前で改行し、該当フレーズを5枚制限注釈と同じ赤色に統一

## [0.11.11] - 2026-07-07

### Changed
- **OCR・管理画面**: 外部アップロードリンク発行パネルを画像アップロード欄の左隣に配置（横並びレイアウト）

## [0.11.10] - 2026-07-07

### Changed
- **OCR・管理画面アップロード**: 枚数上限なし（5枚制限は外部共有URLのみ）。アップロード欄を外部リンク発行より上に配置し、注釈の対象を明確化

## [0.11.9] - 2026-07-07

### Changed
- **OCR・外部アップロード**: 一度に最大5枚まで複数選択してアップロード可能に。失敗があっても残りは続行し、「成功 N/M　失敗 N/M」と失敗理由を表示
- **OCR・外部アップロードリンク**: 発行画面とアップロード画面に「5枚まで」の赤文字注釈を追加

## [0.11.8] - 2026-07-07

### Changed
- **OCR・保存データ**: 一覧の偶数行に薄いグレー背景を付与し、行の視認性を改善

## [0.11.7] - 2026-07-07

### Changed
- **OCR・外部アップロード**: お名前を前回入力値として保持（localStorage）
- **OCR・外部アップロード**: 入力欄の左余白を調整し読みやすく改善
- **OCR・外部アップロード**: 画像種別をOCRで自動判別（手動選択を廃止）。Paygate SSは決済方法なしの場合は従来どおり受付拒否
- **OCR・外部アップロード**: 受付エラー（決済方法なし等）もオーバーレイで表示

## [0.11.6] - 2026-07-07

### Changed
- **OCR・アップロード済み画像**: 入稿者名を表示（管理者アップロードは「管理者」、外部は入力名）
- **OCR・保存データ**: 管理者アップロードの入稿者表示を「管理者」に統一
- **OCR・外部アップロード**: 画像の種類を毎回選択必須に（前回選択を引き継がない）
- **OCR・外部アップロード**: アップロード中に全画面オーバーレイで進行状態を表示

## [0.11.5] - 2026-07-07

### Changed
- **OCR・外部アップロード**: お名前を必須入力に変更（フロント・API双方で検証）
- **OCR・保存データ**: 入稿者名の列を追加（外部アップロードは入力名、管理画面アップロードはユーザー名を表示）

## [0.11.4] - 2026-07-07

### Fixed
- **OCR・外部アップロードリンク**: フォームを縦並び（ラベル → メモ → 有効期限＋発行ボタン）に整理し、横幅を半分程度に制限
