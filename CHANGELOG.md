...
### Changed
- **OCR**: Paygateスクリーンショットと精算レシートを `/operations/ocr` に統合し、タブで切り替え（旧URLはリダイレクト）
- **OCR・外部アップロードリンク**: フォームを2列レイアウトに整理（ラベル・有効期限・メモを半幅配置）。初期表示の画像種別は常に「選択必須」

## [0.11.2] - 2026-07-07

### Fixed
- **OCR・外部アップロードリンク**: 「リンクを発行」ボタンが画面幅いっぱいに伸びる表示を、登録リンク発行画面と同様のコンパクトなボタンに修正

## [0.11.1] - 2026-07-07

### Changed
- **デプロイ**: sudo 不可時に `restart_uvicorn.sh` と OCR ワーカー crontab（5分監視 + @reboot）を自動設定
- **デプロイ**: `.env` に OCR 外部アップロード用シークレットを未設定時のみ自動追記

## [0.11.0] - 2026-07-06

### Added
- **OCR外部アップロード**: 共有リンク発行・公開アップロードページ（fire-and-forget 受付、セッション交換）
- Paygate SS の決済方法プレチェック（best-effort、422 で再アップロード促し）
- OCR 解析ジョブの常駐ワーカー（`vanzai-ocr-worker.service`）
- OCR 画像一覧に外部アップロード由来の「外部」バッジ表示

## [0.10.78] - 2026-07-06

### Fixed
- **OCR・PaygateSS**: 画像解析のポップアップ（通知・進捗ホバー）に、決済方法なし等の具体的なエラーメッセージを表示（「失敗しました」のみでは誤解しやすい問題を改善）

## [0.10.77] - 2026-07-06

### Fixed
- **OCR・PaygateSS**: 画像に「決済方法」の記載がない場合、誤った画像アップロードと判断し、再アップロードを促すメッセージを表示して解析を拒否

## [0.10.76] - 2026-07-06

### Fixed
- **OCR・精算レシート**: 「全件CSV」「精算レシートCSV（拡張）」がダウンロードできない問題を修正（API ルートの登録順序）。精算画面では source_type で絞り込み、失敗時は画面にエラー表示

## [0.10.75] - 2026-07-06

### Fixed
- **OCR・精算レシート**: 小計 > 合計となる矛盾（合計が現金売上を誤拾い等）を検出し、合計を小計で補正して PAYGATE POS を再計算するロジックを追加

## [0.10.74] - 2026-07-06

### Changed
- **OCR・保存データ**: 確定済み行の検証欄「確定」の文字色を深緑に変更

## [0.10.73] - 2026-07-06

### Fixed
- **OCR・精算レシート**: 本番 OCR で `2026107104`（10桁）や `2026/070423:04:34`（日付時刻結合）になる場合に精算日時が欠落する問題を修正
- **OCR・保存データ**: 確定済み行の検証欄を「OK」から緑色の「確定」に変更

## [0.10.72] - 2026-07-06

### Fixed
- **OCR・精算レシート**: 精算日時が `202607104` / `2310434` のように区切りなしで OCR される場合に日付・時刻が欠落する問題を修正（9桁日付の1桁ノイズ除去、日時専用 OCR 帯の追加）
- **OCR・精算レシート**: 確定済み行の検証欄を「OK」から「確定済」に変更

### Added
- **ドキュメント**: 精算レシート OCR 精度向上の施策まとめ `docs/ops/OCR_SETTLEMENT_ACCURACY_PLAYBOOK.md`

## [0.10.71] - 2026-07-06

### Changed
- **OCR・精算レシート**: 精算日の画面表示を `YYYY/MM/DD` に変更（内部・OCR読取は従来どおり ISO / スラッシュ基準）

## [0.10.70] - 2026-07-06

### Changed
- **デプロイ**: `02_app_deploy.sh` を最適化（`pyproject.toml` 未変更時は pip install スキップ、OpenCV 互換時は再インストールスキップ、sudo 不可時は自動で `restart_uvicorn.sh`）
- **デプロイ手順**: VPS では `git pull` / `stash` 不要（スクリプト内 `fetch` + `reset --hard` のみ）。ドキュメント・エージェントルールを統一

## [0.10.69] - 2026-07-06

### Fixed
- **OCR・精算レシート**: 精算日の画面表示を従来どおり `YYYY-MM-DD` に戻す（OCR はレシート表記 `YYYY/MM/DD` を基準に読取）
...
- **OCR・精算レシート**: 端末識別番号・端末番号・精算日時の誤読を改善（`9810`/`9sf0` 系、金額ノイズ、日時形式）。日付 OCR はレシートの `YYYY/MM/DD` 表記を基準に読取
